...
mod tests {
    ...
    async fn test_new_client_insecure_invalid_endpoint() {
        // tonic's insecure path uses connect_lazy(), so construction against a
        // loopback endpoint with nothing listening succeeds without panicking;
        // the connection is only attempted on the first RPC.
    ...
    async fn test_bearer_token_format() {
        // "Bearer <token>" over the wire is covered end-to-end by
        // insecure_host_guard::loopback_allows_insecure_with_no_opt_in_and_sends_token
        // below. This test only checks that a token which cannot be encoded as
        // an HTTP header value is rejected rather than passed through.
