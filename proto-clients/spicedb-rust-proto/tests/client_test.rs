...
mod tests {
    ...
    async fn test_new_client_insecure_invalid_endpoint() {
        // Insecure (plaintext) connections use tonic's connect_lazy, so
        // construction succeeds even though nothing listens on port 0 -- only
        // the first RPC would surface a connection error.
    ...
    async fn test_bearer_token_format() {
        // A token holding a control character has no valid encoding as an
        // HTTP header value, so construction must reject it rather than
        // silently mangling the "Bearer <token>" value or panicking. See
        // `insecure_host_guard::loopback_allows_insecure_with_no_opt_in_and_sends_token`
        // below for the corresponding over-the-wire proof that a well-formed
        // token is sent as exactly "Bearer <token>".
        let result = SpiceDBProtoClient::new("localhost:0", "bad-token\n", true).await;
        assert!(matches!(
            result,
            Err(SpiceDBProtoClientError::InvalidToken(_))
        ));
    }
}
