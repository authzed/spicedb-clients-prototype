...
mod tests {
    ...
    /// Insecure construction uses `connect_lazy`, so it must succeed even
    /// against an endpoint nothing is listening on -- no connection is
    /// attempted until the first RPC.
    #[tokio::test]
    ...
    /// A token that cannot be encoded as an HTTP header value must be
    /// rejected at construction time. The over-the-wire proof that a valid
    /// token is sent as "Bearer <token>" lives in
    /// `insecure_host_guard::loopback_allows_insecure_with_no_opt_in_and_sends_token`.
    #[tokio::test]
    async fn test_bearer_token_format() {
        let result = SpiceDBProtoClient::new("localhost:0", "bad\ntoken", true).await;
        assert!(matches!(
            result,
