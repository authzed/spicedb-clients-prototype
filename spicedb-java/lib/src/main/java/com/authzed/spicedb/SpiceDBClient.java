 ...
 *
 * <p>Implements {@link AutoCloseable} for use with try-with-resources. All
 * streaming methods return {@link Stream} instances that should also be closed
 * when done.
 *
...
public final class SpiceDBClient implements AutoCloseable {
     ...
     */
    public List<Boolean> checkPermissions(Consistency consistency, String permission, Relationship... relationships) {
    ...
    public String deleteRelationships(Filter filter) {
        String revision = "";
        while (true) {
            DeleteRelationshipsResponse resp = withRetry(() ->
    ...
    /**
     * Returns a stream over resource IDs of the given type that the subject
     * has the specified permission on. Cursors are handled transparently.
     *
     * <p>The returned stream should be closed when done.
     */
    public Stream<String> lookupResources(Consistency consistency,
            String resourceType, String permission, String subjectType, String subjectID) {
        Iterator<String> iterator = new Iterator<>() {
            ...
            private void fetchNextPage() {
                var reqBuilder = LookupResourcesRequest.newBuilder()
                    .setConsistency(consistency.toProto())
                ...
                var serverStream = withRetry(() ->
                    permissionsStub.lookupResources(reqBuilder.build())
                );
    ...
    /**
     * Returns a stream over subject IDs of the given type that have the
     * specified permission on the resource. Unlike lookupResources, this does
     * not use cursor-based pagination (not supported in SpiceDB yet) and
     * streams all results in a single call.
     *
     * <p>The returned stream should be closed when done.
     */
    public Stream<String> lookupSubjects(Consistency consistency,
    ...
    public SchemaResult readSchema() {
        ReadSchemaResponse resp = withRetry(() ->
            schemaStub.readSchema(ReadSchemaRequest.getDefaultInstance())
        );
    ...
    public String writeSchema(String schema) {
        WriteSchemaResponse resp = withRetry(() ->
    ...
    public record SchemaCaveat(
        String name, String comment, String expression,
        List<SchemaCaveatParameter> parameters
    ) { }
    ...
    public ReflectSchemaResult reflectSchema(Consistency consistency) {
        ReflectSchemaResponse resp = withRetry(() ->
        ...
        for (var def : resp.getDefinitionsList()) {
            ...
            for (var rel : def.getRelationsList()) {
                relations.add(new SchemaRelation(
                    rel.getName(), rel.getComment(), rel.getParentDefinitionName()));
            }
            var permissions = new ArrayList<SchemaPermission>();
            for (var perm : def.getPermissionsList()) {
                permissions.add(new SchemaPermission(
        ...
        for (var cav : resp.getCaveatsList()) {
            ...
            for (var param : cav.getParametersList()) {
                params.add(new SchemaCaveatParameter(
                    param.getName(), param.getType(), param.getParentCaveatName()));
            }
            caveats.add(new SchemaCaveat(
                cav.getName(), cav.getComment(), cav.getExpression(), List.copyOf(params)));
        }

        return new ReflectSchemaResult(
            List.copyOf(definitions), List.copyOf(caveats),
            resp.getReadAt().getToken());
    }

    /** Identifies a relation or permission on a definition. */
    public record RelationReference(String definitionName, String relationName, boolean isPermission) { }
    ...
    public ComputablePermissionsResult computablePermissions(
        ...
        ComputablePermissionsResponse resp = withRetry(() ->
    ...
    public DependentRelationsResult dependentRelations(
        ...
        DependentRelationsResponse resp = withRetry(() ->
    ...
    public DiffSchemaResult diffSchema(Consistency consistency, String comparisonSchema) {
        DiffSchemaResponse resp = withRetry(() ->
    ...
    public ExpandResult expandPermissionTree(
        ...
        ExpandPermissionTreeResponse resp = withRetry(() ->
    ...
    public long importRelationships(Iterable<Relationship> relationships) {
        ...
        StreamObserver<ImportBulkRelationshipsResponse> responseObserver = new StreamObserver<>() {
            @Override
        ...
        for (Relationship r : relationships) {
            ...
            if (batch.size() >= DEFAULT_IMPORT_BATCH_SIZE) {
                requestObserver.onNext(
                    ImportBulkRelationshipsRequest.newBuilder()
    ...
    /**
     * Returns a stream over all relationships matching the optional filter,
     * streamed from SpiceDB in bulk. Cursors are handled transparently with
     * 512-item pages.
     *
     * <p>The returned stream should be closed when done.
     */
    public Stream<Relationship> exportRelationships(Consistency consistency, Filter filter) {
        Iterator<Relationship> iterator = new Iterator<>() {
            private Cursor cursor = null;
            ...
            private void fetchNextPage() {
                ...
                var reqBuilder = ExportBulkRelationshipsRequest.newBuilder()
                    .setConsistency(consistency.toProto())
    ...
    /**
     * Returns a stream over relationship changes from SpiceDB's watch API,
    ...
    public Stream<Update> updates(List<String> objectTypes, String startRevision) {
        ...
        if (startRevision != null && !startRevision.isEmpty()) {
            reqBuilder.setOptionalStartCursor(
                ZedToken.newBuilder().setToken(startRevision).build());
        }

        var serverStream = withRetry(() -> watchStub.watch(reqBuilder.build()));

        Iterator<Update> iterator = new Iterator<>() {
    ...
    public CountResult experimentalCountRelationships(String name) {
        ExperimentalCountRelationshipsResponse resp = withRetry(() ->
    ...
    private Stream<Relationship> paginatedRelationshipStream(
        ...
        Iterator<Relationship> iterator = new Iterator<>() {
            ...
            private void fetchNextPage() {
                ...
                var reqBuilder = ReadRelationshipsRequest.newBuilder()
                    .setConsistency(consistency.toProto())
                    .setRelationshipFilter(toRelationshipFilter(filter))
                ...
                var serverStream = withRetry(() ->
                    permissionsStub.readRelationships(reqBuilder.build())
                );
    ...
    private static RelationshipUpdate toRelationshipUpdate(Transaction.Mutation m) {
        RelationshipUpdate.Operation op = switch (m.operation()) {
    ...
    private static Precondition toPrecondition(Transaction.Precondition p) {
        Precondition.Operation op = switch (p.operation()) {
    ...
    static com.authzed.api.v1.Relationship toProtoRelationship(Relationship r) {
        ...
        if (r.caveatName() != null && !r.caveatName().isEmpty()) {
            var caveatBuilder = ContextualizedCaveat.newBuilder()
                .setCaveatName(r.caveatName());
    ...
    static Relationship fromProtoRelationship(com.authzed.api.v1.Relationship pr) {
        ...
        if (pr.hasOptionalExpiresAt()) {
            expiration = Instant.ofEpochSecond(
                pr.getOptionalExpiresAt().getSeconds(),
                pr.getOptionalExpiresAt().getNanos()
            );
        }
    ...
    private static RelationshipFilter toRelationshipFilter(Filter f) {
        var builder = RelationshipFilter.newBuilder()
        ...
        if (f.subjectType() != null && !f.subjectType().isEmpty()) {
            var subjectBuilder = SubjectFilter.newBuilder()
                .setSubjectType(f.subjectType());
            ...
            if (f.subjectRelation() != null && !f.subjectRelation().isEmpty()) {
                subjectBuilder.setOptionalRelation(
                    SubjectFilter.RelationFilter.newBuilder()
                        .setRelation(f.subjectRelation())
                        .build());
            }
    ...
    private Update updateFromProto(RelationshipUpdate pu) {
        UpdateOperation op = switch (pu.getOperation()) {
    ...
    private SchemaDiff schemaDiffFromProto(ReflectionSchemaDiff d) {
        ...
        if (d.hasDefinitionAdded()) {
            ...
            return new SchemaDiff("definition_doc_comment_changed", d.getDefinitionDocCommentChanged().getName(), "", "", "");
        } else if (d.hasRelationAdded()) {
            return new SchemaDiff("relation_added", d.getRelationAdded().getParentDefinitionName(), d.getRelationAdded().getName(), "", "");
        } else if (d.hasRelationRemoved()) {
            return new SchemaDiff("relation_removed", d.getRelationRemoved().getParentDefinitionName(), d.getRelationRemoved().getName(), "", "");
        } else if (d.hasRelationDocCommentChanged()) {
            return new SchemaDiff("relation_doc_comment_changed", d.getRelationDocCommentChanged().getParentDefinitionName(), d.getRelationDocCommentChanged().getName(), "", "");
        } else if (d.hasRelationSubjectTypeAdded()) {
            return new SchemaDiff("relation_subject_type_added", d.getRelationSubjectTypeAdded().getRelation().getParentDefinitionName(), d.getRelationSubjectTypeAdded().getRelation().getName(), "", "");
        } else if (d.hasRelationSubjectTypeRemoved()) {
            return new SchemaDiff("relation_subject_type_removed", d.getRelationSubjectTypeRemoved().getRelation().getParentDefinitionName(), d.getRelationSubjectTypeRemoved().getRelation().getName(), "", "");
        } else if (d.hasPermissionAdded()) {
            return new SchemaDiff("permission_added", d.getPermissionAdded().getParentDefinitionName(), "", d.getPermissionAdded().getName(), "");
        } else if (d.hasPermissionRemoved()) {
            return new SchemaDiff("permission_removed", d.getPermissionRemoved().getParentDefinitionName(), "", d.getPermissionRemoved().getName(), "");
        } else if (d.hasPermissionDocCommentChanged()) {
            return new SchemaDiff("permission_doc_comment_changed", d.getPermissionDocCommentChanged().getParentDefinitionName(), "", d.getPermissionDocCommentChanged().getName(), "");
        } else if (d.hasPermissionExprChanged()) {
            return new SchemaDiff("permission_expr_changed", d.getPermissionExprChanged().getParentDefinitionName(), "", d.getPermissionExprChanged().getName(), "");
            ...
            return new SchemaDiff("caveat_doc_comment_changed", "", "", "", d.getCaveatDocCommentChanged().getName());
            ...
            return new SchemaDiff("caveat_parameter_added", "", "", "", d.getCaveatParameterAdded().getParentCaveatName());
        } else if (d.hasCaveatParameterRemoved()) {
            return new SchemaDiff("caveat_parameter_removed", "", "", "", d.getCaveatParameterRemoved().getParentCaveatName());
        } else if (d.hasCaveatParameterTypeChanged()) {
            return new SchemaDiff("caveat_parameter_type_changed", "", "", "", d.getCaveatParameterTypeChanged().getParameter().getParentCaveatName());
        }
