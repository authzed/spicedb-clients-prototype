...
class SpiceDBClient:
    ...
    def lookup_resources(
        ...
        *,
        ...
        Results are **not guaranteed to be unique** — the same resource may
        be yielded more than once (e.g. via caveated/conditional results),
        possibly with differing ``permissionship``. Callers that require
        uniqueness should deduplicate by ``resource_id``.
        """
        self._ensure_channel()
    ...
    def lookup_subjects(
        ...
        *,
        ...
        Results are **not guaranteed to be unique** — the same subject may
        be yielded more than once, possibly with differing
        ``permissionship``. Callers that require uniqueness should
        deduplicate by ``subject.subject_id``.
        """
        self._ensure_channel()
        ctx_struct = _requests.context_struct(context)
