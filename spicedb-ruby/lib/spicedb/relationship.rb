...
module SpiceDB
  ...
  Relationship = Data.define(
    ...
    def self.from_tuple(tuple)
      ...
      raise SpiceDB::InvalidArgumentError, "invalid tuple format: missing '#' in resource" unless resource_parts.length == 2

      resource_type_id = resource_parts[0].split(':', 2)
      raise SpiceDB::InvalidArgumentError, "invalid tuple format: missing ':' in resource type:id" unless resource_type_id.length == 2

      subject_hash = subject_part.split('#', 2)
      subject_type_id = subject_hash[0].split(':', 2)
      raise SpiceDB::InvalidArgumentError, "invalid tuple format: missing ':' in subject type:id" unless subject_type_id.length == 2
